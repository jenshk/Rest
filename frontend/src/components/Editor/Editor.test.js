...
beforeEach(()=>{
    ...
    wrapper = mount( <Provider store={store}><Editor /></Provider> );
})
...
it('Onchange sets state', () => {
    ...
    wrapper.find('input[name="firstname"]').simulate('change', {target: {value: 'abc', name: 'firstname'}});
    expect(wrapper.find('Editor').state('firstname')).toEqual('abc');
});
